...
def parse_args():
    ...
    parser.add_argument('--prediction', type=str, help='result file', default="/scratch/generalvision/mowentao/kuri3d-output/fuyu-8b-scanqa-2024-05-09-14-55-2024-05-09-14-55/ckpt-0/pred/scanqa.json")
    parser.add_argument('--annotation', type=str, help='annotation file', default="/scratch/generalvision/mowentao/ScanQA/data/qa/ScanQA_v1.0_val.json")
...
def main(args):
    ...
    assert len(prediction) == len(annotation), 'The number of prediction and annotation must be the same'

    annotation_dict = {}
    for item in annotation:
        annotation_dict[item['question_id']] = [clean_answer(answer.strip()) for answer in item['answers']]

    for qid, preds in prediction.items():
        preds = [clean_answer(pred) for pred in preds]
        prediction[qid] = preds

    score_per_caption, metric_message, metrics_epoch = score_captions(annotation_dict, prediction)

    # print(score_per_caption)
