...
wandb_project = "Kuri3D-captioning"
...
def parse_args():
    ...
    parser.add_argument("--best_criteria", type=str, default="CiDEr")
...
def get_trainval_datasets(args) -> dict[str, Dataset]:
    if args.dataset == "scan2cap":
        assert args.use_3d
        train_dataset = Scan2CapSimpleDataset(
            name=args.dataset,
            split="train",
        ...
        val_dataset = Scan2CapSimpleDataset(
            name=args.dataset,
            split="val",
            ratio=args.train_ratio,
            use_color=True,
...
def collate_fn_eval(examples) -> dict:
    to_copy_keys = ["question_id", "target", "scan2cap_id"]
    ...
    # output["target"] = texts
    # output["question_id"] = [e["question_id"] for e in examples]
...
def validate(args, epoch):
    ...
    for step, batch in enumerate(args.dataloaders["val"]):
        with torch.no_grad():
            ...
            question_id = deepcopy(batch["question_id"])
            scan2cap_id = deepcopy(batch["scan2cap_id"])
            del batch["target"]
            del batch["question_id"]
            del batch["scan2cap_id"]
            ...
            # record prediction               
            # for qid, pred, t in zip(question_id, pred_answer, target_text):
            for qid, pred, sample_id in zip(question_id, pred_answer, scan2cap_id):
                preds[qid] = [pred]
                target_texts[qid] = args.datasets["val"].corpus[sample_id] # supposedly a list of texts
