...
from utils.pc_utils import scale_points, shift_scale_points
...
class Model_Vote2Cap_DETR(nn.Module):
    ...
    def __init__(
        ...
        encoder: Union[TransformerEncoder, MaskedTransformerEncoder],
    ...
    def forward(self, inputs, is_eval: bool=False):
        ...
        # logging.info(f"encoder features: {box_predictions['outputs']['enc_features'].shape}")
        return box_predictions['outputs']
        # return enc_features, torch.ones(*enc_features.shape[:-1]).to(enc_features)




def build_preencoder(cfg) -> PointnetSAModuleVotes:
...
def build_encoder(cfg) -> Union[TransformerEncoder, MaskedTransformerEncoder]:
...
def build_decoder(cfg) -> TransformerDecoder:
...
def detector(args, dataset_config) -> Model_Vote2Cap_DETR:
