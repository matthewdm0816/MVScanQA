"""
Adapted reference: Honeybee project from https://github.com/kakaobrain/honeybee/
"""
...
# class ProjectorConfig(dict):
#     def __init__(self, *args, **kwargs):
#         super(ProjectorConfig, self).__init__(*args, **kwargs)
#         self.__dict__ = self
...
#             layer_norm_eps=1e-6,  #
#             encoder_hidden_size=1024,  # This will be overwritten by vision_model's hidden_size
#             pos_emb=False,
...
def build_pos_embeds(
    config: ProjectorConfig, num_input_tokens: int, vision_hidden_size: int
):
    # pos emb
    if config.pos_emb:
...
def build_eos_tokens(config: ProjectorConfig, output_hidden_size: int):
    # think tokens
...
def build_prenorm(config: ProjectorConfig):
    if config.prenorm:
...
class DAbstractor(DeformableDetrDecoder):
    ...
    def __init__(
        ...
        if self.pooled_v_target != "none":
            tgt_hw = int(config.num_queries ** (1 / 3))
            # self.downsampler = nn.AdaptiveAvgPool2d((tgt_hw, tgt_hw))
            self.downsampler = nn.AdaptiveAvgPool3d((tgt_hw, tgt_hw, tgt_hw))
