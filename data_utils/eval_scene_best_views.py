...
DSET_PATH_SCANQA = {
    "test_w_obj": f"{DATA_PATH}/ScanQA_v1.0_test_w_obj.json",
    "test_wo_obj": f"{DATA_PATH}/ScanQA_v1.0_test_wo_obj.json",
    "train": f"{DATA_PATH}/ScanQA_v1.0_train.json",
    "val": f"{DATA_PATH}/ScanQA_v1.0_val.json",
}

DSET_PATH_SCANQA_MV = {
    "train": f"{DATA_PATH}/qa/ScanQA_mv_train_filtered_cleaned.json",
    "val": f"{DATA_PATH}/qa/ScanQA_mv_val_filtered_cleaned.json",
}

DSET_PATH_SQA = {
    "test": f"{DATA_PATH}/SQA_test.json",
    "train": f"{DATA_PATH}/SQA_train.json",
    "val": f"{DATA_PATH}/SQA_val.json",
}
...
if __name__ == "__main__":
    ...
    import os, sys, json, glob, logging, random, pickle, warnings, colorama, toml, transformers, datasets
    ...
    def parse_args():
        ...
        parser.add_argument("--outfile", type=str, default="scene_eval.pth")
        ...
        parser.add_argument("--answer_freq_threshold", type=int, default=0)
        parser.add_argument("--not_eval_vqa", action="store_true")
        parser.add_argument("--dset_views_path", type=str, default=DSET_VIEWS_PATH)
        parser.add_argument("--max_answer_count", type=int, default=5000)
    ...
    elif args.dataset == "scanqa_mv":
        DSET_PATH = DSET_PATH_SCANQA_MV
    ...
    for split in args.split:
        dset[split] = dset[split].remove_columns(list({"object_ids", "object_names", "n_views_can_solve", "best_view_overlap_mean", "best_view_overlap_min", "best_view_overlap_max"}.intersection(dset[split].column_names)))
    ...
    SCAN_NAMES = sorted(set(all_scans))
