"""
modified from PyPI `tokenizer-changer` package to fit UniGram tokenizer models
"""
...
# ================== Delete operations ==================
    ...
    def delete_tokens(self, unwanted_tokens: list[str] = [], include_substrings: bool = True, delete_merges: bool = True):
        ...
        elif isinstance(self.state["model"]["vocab"], list):
            # it is a UniGram model
            # so, it is a list of ["token", logprob], so we need to find the index of the token
            # index = [i for i, x in enumerate(
                # self.state["model"]["vocab"]) if x[0] == token][0]
            # print(index)
            # del self.state["model"]["vocab"][index]
            token_to_index = {item[0]: (idx, item) for idx, item in enumerate(self.state["model"]["vocab"])}
            unwanted_tokens_set = set(self.unwanted_tokens)
            remained_token_to_index = {k: v for k, v in token_to_index.items() if k not in unwanted_tokens_set}
            self.state["model"]["vocab"] = [item for (idx, item) in sorted(remained_token_to_index.values(), key=lambda x: x[0])]
